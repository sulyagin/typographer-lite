export default class typographerLite {
  ...
  parse() {
    ...
    while (currentNode = nodes.nextNode()) {
      Object.keys(this.rules()).forEach(key => currentNode.nodeValue = this.rules()[key](currentNode.nodeValue));
    }
  }
  rules() {
    return {
  ...
  format(str) {
    if(typeof str !== 'string')
      return false;
    Object.keys(this.rules()).forEach(key => str = this.rules()[key](str));
    return str;
  }
}